...
app.set('view engine', 'html');
    // Express 애플케이션에서 사용할 뷰 엔진을 설정

// Nunjucks 템플릿 엔지을 설정
nunjucks.configure('views', {
    //'views' -> 템플릿 파일이 위치한 디텍토리 경로 지정
    express: app,
    //Nunjucks가 Express의 템플릿 엔진으로 사용되 때, eXPRESS 애플리케이션과 연동될 수 있도록 설정
    watch: true,
    // 템플릿 파일의 벼경 -> Nunjucks 자동업데이트 수행
...
app.use((err, req, res, next) => {
    ...
    // process.env.NODE.ENV -> 현재 실행 환경을 나타내는 node.js의 횐경 변수
    //여러 객체를 'production'(배포) 환경이 아닌 경우(개발환경 또는 테스트 환경)에만 로컬 변수에 할당
